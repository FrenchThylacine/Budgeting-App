...
The three aircraft (`assets/brand/{concorde,a350,alphajet}-source.jpg`) are cut off their sky by the same script and turned nose-right, and are the loading sequence's full-colour artwork. The tab transition flies a separate fleet: twenty-two white silhouettes extracted from the supplied Flightradar24 icon sheet by `scripts/extract-craft.mjs`, each rotated nose-right and fitted to one 160px box. `domain/aircraft.ts` holds both lists — `AIRCRAFT` and `FLEET` — and both dimensions come from the asset, so nothing reflows as an image decodes.
