...
**One vocabulary for who paid.** `FundingMark` and `CadenceMark` are the only two components that draw these marks, and a test asserts it: three hand-written copies of the same three glyphs is how a vocabulary quietly stops being one. The three funding states have one identity everywhere they appear — a badge on a row, a series in a chart, a chip on a card, a column in the report: the **accent** for money out of this budget, **blue** for somebody else's, **amber** for money you keep outside it. Each also carries a glyph (● ◆ ▲) and a word, because colour is the fastest channel and never the only one: the states survive a greyscale printer and a colour-blind reader, and the report's three inks are separated by lightness as well as hue so that two blues do not print as one grey.

**And one for how often.** Seven cadences — monthly, yearly, weekly, scheduled, per session, session pack, one-off — each with a shape, a tone and a word, defined once in `domain/cadence.ts`. The tones are three, not seven: what is on a calendar the app knows, what is counted by how often you turn up, and what happened once. The family carries most of the meaning, and the silhouettes differ rather than the details, because at fourteen pixels two calendars are one calendar. It replaced six ways of writing the same fact, one of which was the stored enum value printed straight onto a transaction row.

**Two equivalents, two questions.** Under a *record* — a transaction in a currency of its own — the "≈" line is the **display currency**, the one every total on the page is already in: `150 000 LBP ≈ €1.35`. Under an *aggregate* — a wallet balance, a month's activity cost — it is the optional **second currency**, for somebody who earns in one currency and budgets in another: `€2,400 ≈ $2,790`. They are different questions and they are different functions; one function answering both is how a Lebanese taxi in a euro budget came to print "≈ $1.47".
...
**The loading screen is a formation, in three dimensions.** The chosen aircraft holds the centre while two Alpha Jets fly a routine around it, one trailing blue smoke and one red. The track is a circle in a plane tilted 56° out of the screen — so they pass over the lead, under it, in front of its nose and away behind its tail, with perspective, occlusion and a little aerial haze all taken from the same `z` — and three incommensurate harmonics ride on top of it: the plane rolls, the track climbs, the radius breathes. One circle at a constant rate reads as machinery however three-dimensional it is. When the data is ready they roll out of the turn and form up behind it, a third joins trailing white, the three ribbons settle into a tricolour, and the whole formation accelerates away to the right, taking the loading screen with it and uncovering the application.
...
- **The browser harness covers the workflows, not the whole surface.** `npm run verify` drives a real Chrome through the DevTools protocol — the loading sequence, every theme, the aircraft, the transition's direction, the period selector's layering, the exchange-rate refresh and its pair sheet, building an activity and a transaction, the funding split, the wallet and its reset, the report, the loading routine measured rather than asserted — the distance from a sprite's own transform to the nearest pixel of its own smoke, the variation in the track's radius, and how far the third jet flies to join the formation — and sweeps for overflow, small targets and WCAG AA contrast at six phone widths and 1440px in both a light and a dark theme. It does not open every editor or every dialog. A run that dies part-way now exits non-zero and says so: it used to print "0/0 checks passed" and report success.
